...
async def main():
    ...
    # initialise the client: creamos una instancia del cliente antes de comenzar a utilizarlo en un entorno asíncrono.
    client = await AsyncClient.create()             #utilizamos el módulo AsyncClient de la biblioteca python-binance para interactuar con la API de Binance en un entorno asincrónico

    # run some simple requests
    #print(json.dumps(await client.get_exchange_info(), indent=2))   #Obtenemos información del exchange de forma asincrónica, json.dumps convierte un objeto de Python a una cadena JSON 

    print(json.dumps(await client.get_symbol_ticker(symbol="BTCUSDT"), indent=2))

    # initialise websocket factory manager
    bsm = BinanceSocketManager(client)      #comunicación interactiva en tiempo real
    ...
    klines = client.get_historical_klines("ETHBTC", AsyncClient.KLINE_INTERVAL_30MINUTE, "1 Dec, 2017", "1 Jan, 2018")

    # fetch weekly klines since it listed
    klines = client.get_historical_klines("NEOBTC", AsyncClient.KLINE_INTERVAL_1WEEK, "1 Jan, 2017")
